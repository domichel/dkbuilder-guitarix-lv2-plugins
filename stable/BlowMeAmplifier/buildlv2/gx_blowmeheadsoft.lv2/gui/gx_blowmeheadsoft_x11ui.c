...
static void resize_event(gx_blowmeheadsoftUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_blowmeheadsoftUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_blowmeheadsoftUI* ui = (gx_blowmeheadsoftUI*)malloc(sizeof(gx_blowmeheadsoftUI));
...
static void cleanup(LV2UI_Handle handle) {
	gx_blowmeheadsoftUI* ui = (gx_blowmeheadsoftUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_blowmeheadsoftUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_blowmeheadsoftUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_blowmeheadsoftUI *ui, gx_controller* controller) {
...
static void _expose(gx_blowmeheadsoftUI *ui) {
	const char* plug_name = "GxBlowMeHeadSoft" ;
...
static void controller_expose(gx_blowmeheadsoftUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_blowmeheadsoftUI *ui, int controller) {
...
static void check_value_changed(gx_blowmeheadsoftUI *ui, int i, float* value) {
...
static void check_is_active(gx_blowmeheadsoftUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_blowmeheadsoftUI *ui) {
...
static bool get_active_ctl_num(gx_blowmeheadsoftUI *ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_blowmeheadsoftUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_blowmeheadsoftUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_blowmeheadsoftUI *ui, int i) {
	float value;
...
static void switch_event(gx_blowmeheadsoftUI *ui, int i) {
	float value = ui->controls[i].adj.value ? 0.0 : 1.0;
	check_value_changed(ui, i, &value);
}

// left mouse button is pressed, generate a switch event, or set controller active
static void button1_event(gx_blowmeheadsoftUI *ui, double* start_value) {
	int num;
	if (get_active_ctl_num(ui, &num)) {
...
static void motion_event(gx_blowmeheadsoftUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_blowmeheadsoftUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_blowmeheadsoftUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_blowmeheadsoftUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_blowmeheadsoftUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_blowmeheadsoftUI *ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void event_handler(gx_blowmeheadsoftUI *ui) {
	XEvent xev;
