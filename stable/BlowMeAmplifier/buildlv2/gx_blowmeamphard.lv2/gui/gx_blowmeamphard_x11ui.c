...
static void resize_event(gx_blowmeamphardUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_blowmeamphardUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_blowmeamphardUI* ui = (gx_blowmeamphardUI*)malloc(sizeof(gx_blowmeamphardUI));
	...
	ui->controls[2] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {180, 30, 61, 61}, false,"MASTER", KNOB, MASTER};
...
static void cleanup(LV2UI_Handle handle) {
	gx_blowmeamphardUI* ui = (gx_blowmeamphardUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_blowmeamphardUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_blowmeamphardUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_blowmeamphardUI *ui, gx_controller* controller) {
...
static void _expose(gx_blowmeamphardUI *ui) {
	const char* plug_name = "GxBlowMeAmpHard" ;
...
static void controller_expose(gx_blowmeamphardUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_blowmeamphardUI *ui, int controller) {
...
static void check_value_changed(gx_blowmeamphardUI *ui, int i, float* value) {
...
static void check_is_active(gx_blowmeamphardUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_blowmeamphardUI *ui) {
...
static bool get_active_ctl_num(gx_blowmeamphardUI *ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_blowmeamphardUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_blowmeamphardUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_blowmeamphardUI *ui, int i) {
	float value;
...
static void motion_event(gx_blowmeamphardUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_blowmeamphardUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_blowmeamphardUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_blowmeamphardUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_blowmeamphardUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_blowmeamphardUI *ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void event_handler(gx_blowmeamphardUI *ui) {
	XEvent xev;
